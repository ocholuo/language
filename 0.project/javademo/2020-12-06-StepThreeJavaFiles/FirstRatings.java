...
public class FirstRatings{
    ...
    public ArrayList<Movie> loadMovies(String filename){
        ...
        // System.out.println("FirstRatings.csvMovieme");
    ...
    private ArrayList<Rater> csvRater(CSVParser parser){
        ...
        ArrayList<String> currRatersID = new ArrayList<String>();
        
        for(CSVRecord csvrate : parser){
            String raterID = csvrate.get("rater_id");
            String movieID = csvrate.get("movie_id");
            Double ratingvalue = Double.parseDouble(csvrate.get("rating"));

            if (currRatersID.contains(raterID)) {
                // System.out.println("old rater find: " + raterID);
                int raterindex = currRatersID.indexOf(raterID);

                Rater currRater = raterslist.get(raterindex);
                currRater.addRating(movieID,ratingvalue);
                raterslist.set(raterindex, currRater);
            }
            else {
                // System.out.println("new rater find: " + raterID);
                Rater newRater = new EfficientRater(raterID);
                newRater.addRating(movieID,ratingvalue);
                raterslist.add(newRater);
                currRatersID.add(raterID);
            }
        }
        // System.out.println("read data for " +  raterslist.size() + " raters");
        return raterslist;
    }
    ...
    //                 int raterDataindex = raterData.indexOf(raterID);
    //                 PlainRater rater = raterData.get(raterDataindex);
    //                 rater.addRating(ratingItem, ratingValue);
    //                 raterData.add(raterDataindex, rater);
    //             }
    //             else{
    ...
    //     }
    //     catch(Exception ioe) {
    //         System.out.println("IOException caught in FirstRatings.loadRaters");
    //     }
    //     return raterData;
    // }

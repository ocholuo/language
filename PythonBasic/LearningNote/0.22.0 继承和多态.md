...
# make sure the Cat class inherits from the Pet class
# do that by putting the superclass word Pet in parentheses
class Cat(Pet):
    sounds = ['Meow']
...
# continue the inheritance tree.
# inherits from Cat, which inherits from Pet
class Cheshire(Cat):

    # this method is specific to instances of Cheshire
    def smile(self):
        print(":D :D :D")

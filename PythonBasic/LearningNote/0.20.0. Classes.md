...
- This `initializer method`/`constructor`, is automatically called whenever a new instance of Point is created.
...
- `__init__`: 可以把各种属性绑定到 self 创建的实例本身。
...
each instance of a class has its own namespace with its own instance variables.
...
1. When the interpreter sees an expression of the form `<obj>.<varname>`, it:
...
2. When the interpreter sees an assignment statement of the form `<obj>.<varname> = <expr>`, it:
...
to set the class variable: `<varname> = <expr>` at the top-level in a class definition.
